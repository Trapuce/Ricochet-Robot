...
public class AStarFinalForm extends AStarMax {
	...
	public List<State> aStarFF() {
		...
		Robot specialRobot = state.getSpecialRobot();
		Coord start = specialRobot.getCoord();
		...
		int[][] table = super.heuristicTable1(target);
		...
		while (!openQueue.isEmpty()) {
			...
			for (Move move : listOfMoves) {
				...
				tentativeGScore = gScore.get(currentState) + 5;
				}
